...
function renderSubjects(){
  ...
  sj.innerHTML=names.map(n=>`<article class="subject-card" data-subject="${n}">
    ...
    <small>Choose a year and medium to download</small></article>`).join("");
...
function showPicker(name){
  ...
  for(let y=2026;y>=2000;y--) ys+=`<option value="${y}">${y}</option>`;
  panel.innerHTML=`<div class="direct-card">
    ...
    <div id="status" class="status">Choose a year and medium.</div>
  </div>`;
  ["year","medium"].forEach(id=>document.getElementById(id).onchange=update);
...
function update(){
  ...
  if(!y||!m){area.innerHTML="";status.textContent="Choose year and medium.";return;}
  ...
  area.innerHTML=`<div class="download-box"><div>
    ...
    <small>Downloads the actual document through your website.</small>
  </div><a class="btn btn-primary" href="${url}">Download PDF ↓</a></div>`;
  status.textContent="Click Download PDF. The server will locate the real GovDoc document for this year and medium.";
}
