...
function renderSubjects(){
 ...
 const names=[...new Set(D.streams.flatMap(s=>s.subjects))].filter(n=>D.subjects.includes(n)&&(f==="all"||inStream(n,f))&&(!q||n.toLowerCase().includes(q)));
 empty.classList.toggle("hidden",names.length>0);
 sj.innerHTML=names.map(n=>`<article class="subject-card" data-subject="${n}">
 <div class="top"><h3>${n}</h3><span class="badge">Available</span></div>
 <small>GovDoc paper lookup available</small></article>`).join("");
